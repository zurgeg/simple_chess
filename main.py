...
class Game:
    ...
    def __init__(self):
        ...
        self.WHITEPAWNS_BOOST = []
        for i in range(8):
            self.WHITEPAWNS_BOOST.append(False)
        self.message = "What piece to move"
        self.gameboard = {}
        self.placePieces()
        print("ultra chess. enter moves in algebraic notation separated by space")
        self.main()
...
class Pawn(Piece):
    ...
    def availableMoves(self,x,y,gameboard, Color = None):
        ...
        if (x-1,y+self.direction) in gameboard and self.noConflict(gameboard, Color, x-1, y+self.direction) : answers.append((x-1,y+self.direction))
        if (x,y+self.direction) not in gameboard and Color == self.Color : answers.append((x,y+self.direction))# the condition after the and is to make sure the non-capturing movement is not used in the calculation of checkmate
        return answers
